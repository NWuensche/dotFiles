...
FREE=$(curl -s 'https://www.gamerpower.com/giveaways/pc/free-games' \
  ...
  --compressed \
  |  tr '<' '\n' |  sed -n 's/.*p class="card-text truncate2 text-muted mb-2 mt-1">\(.*\)/\1/p' | grep -iv 'Black Desert Online' | grep -iv 'Microsoft Store' |  grep -iv 'Indiegaka' | grep -iv 'Indiegala' | grep -iv 'Indie Gala' | grep -iv 'itchio' | grep -iv 'itch.io' | head -n 1 | sed -n "/$GAME/p" ) #INFO Don't show anything from Indiegala or itchio
  if [[ "$FREE" == "" ]] ; then
