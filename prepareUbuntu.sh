...
CPUS=$(cat /proc/cpuinfo | sed -n 's/.*\(7840\).*/\1/p' | head -n 1) #Which AMD

HDD="/media/5f65b653-f040-40eb-a2de-64a7e4cac5c4"
...
function yayPackages {
   ...
   # yay -S texlive-basic texlive-latexextra texlive-latexrecommended biber tllocalmgr-git texlive-binextra  texlive-langgerman  --noconfirm --needed #Latex + latexmk TODO Add, but don't download texlive-full. This is downloaded twice here and each round takes >1h 
    ...
    if [[ "$CPU" == "AMD" ]]; then
      ...
      if [[ "$CPUS" == "7840" ]]; then
        #LAN
        sudo systemctl enable --now netctl-ifplugd@enp101s0f4u1u4.service
        sudo systemctl enable netctl-auto@wlp1s0.service
      else
        #LAN
        sudo systemctl enable --now netctl-ifplugd@enp4s0f3u1u4.service #Need a netctl profile for this to work #TODO
...
#   echo "Start Firefox once, close it, remove  .mozilla/firefox/RANDOM.default-release folder completely and move firefoxProfile to it (RANDOM part has to match exactly with old folder!)"
    ...
    echo "bluetuith: TRUST the devices after connecting to them, fixes no-volume-control-issue"
}
